/** Shared modern MCP wire controls. No application, route or deployment authority. */
import { PROTOCOL_VERSION_META_KEY, createMcpHandler, type JSONRPCRequest, type McpHttpHandler, type McpServerFactory } from "@modelcontextprotocol/server";
import { MCP_PROTOCOL_VERSION } from "./mcp-constants.js";
import { containsRawIdempotencyKeyInMcpResourceUri, containsRawIdempotencyKeyInMcpText, isBoundedMcpRequestId, isBoundedMcpResourceUri } from "./mcp-wire-guards.js";
import { withMcpHttpDataQuerySignal } from "./mcp-request-signal.js";
import { BoundedJsonError, parseBoundedJsonBytes } from "./bounded-json.js";
...
export function createBoundedMcpHttpHandler(
  factory: McpServerFactory,
  onerror: ((error: Error) => void) | undefined,
): McpHttpHandler {
  return createBoundedHttpHandler(factory, onerror, "auto");
}

/** Closed hosted experiment mode; no arbitrary SDK options are accepted. */
export function createBoundedJsonOnlyMcpHttpHandler(factory: McpServerFactory): McpHttpHandler {
  return createBoundedHttpHandler(factory, undefined, "json");
}

function createBoundedHttpHandler(
  factory: McpServerFactory,
  onerror: ((error: Error) => void) | undefined,
  responseMode: "auto" | "json",
): McpHttpHandler {
