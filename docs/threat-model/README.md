...
| RK17 provider and storage exhaustion | The generic container remains blocked. The fixed local container re-verifies the complete linked ledger and reconciliation index on every `/readyz` evaluation. New `data.query` keys reserve process concurrency and first-attempt rate capacity before immutable ownership; at the claim ceiling readiness becomes `503 reconciliation-capacity-exhausted` while recovery routes stay mounted. | Both checks are linear in retained events and claims. The 4,096-claim store is a finite local demonstrator; shared deployment still requires identity, per-principal and global quotas, capacity evidence and monitoring. |
...
| RK17 provider exhaustion | The Node ingress propagates direct and MCP disconnects to the adapter and uses 5-second ingress controls with 25-second socket-inactivity headroom around the adapter's lower absolute 20-second complete ceiling. A module-branded, time-bounded no-egress lease reserves process concurrency and first-attempt rate capacity before a new immutable claim. Claim races and capacity rejection release the unused lease; transport starts only after ownership. The local index refuses new ownership before publication at 4,096 claims and readiness then reports the exhaustion truthfully. | Once key ownership exists, cancellation or uncertain execution can leave an immutable pending claim. Successful claims also consume the finite local capacity. The adapter limiter and index bound are still local; shared deployment requires identity, per-principal and global quotas plus governed capacity operations. |
...
## DEPLOY-207 local unregistered gateway container scope

The repository-only image mounts the exact-five candidate locally. The Compose
harness does not register, publish or deploy the service.

| Research risk | Control in this slice | Residual boundary |
| --- | --- | --- |
| RK08 policy bypass and lifecycle confusion | The fixed entrypoint reasserts the blocked generic defaults, verifies the exact snapshot, linked evidence pair and sole T04 record, then passes only those inputs to a closed builder. The builder creates the fixed active ONS adapter and derives the exact ordered five direct and MCP operations plus three resources from one branded assembly. Production registration remains false and readiness fails closed on any subtraction or integrity failure. | Production registration still requires all lifecycle gates and separate reviewed authority. Environment, command-line, path, serialised and arbitrary loader activation remain forbidden. |
| RK10 SSRF and RK17 exhaustion | The fixed adapter retains the exact ONS origin and transport controls. The default Compose network is internal, has one replica and no operator-supplied environment. It declares only `127.0.0.1:8787`; acceptance exercises catalogue-only behaviour and exact discovery without making a provider call. New keys must hold the no-egress provider-admission lease before immutable ownership, and exhausted claim capacity blocks readiness without hiding recovery routes. Process, CPU, memory, swap, descriptor and existing request bounds are inspected on the running container. | A no-port receipt is not host-ingress or provider evidence. A deployed ONS candidate requires domain-aware egress, identity, per-principal and global admission, and an operator response to permanently pending reconciliation claims. |
| RK21 evidence loss or tampering | Separate private ledger and reconciliation volumes are opened and fully verified before listening. Descriptor modes and hashes survive restart and exact-image restore. Closed canonical schemas cover the image receipt, scan, acceptance and final manifest; exactly 24 evidence subjects plus the manifest are allowed, and protected-main provenance can attest that manifest after complete re-verification. A separate inactive provider-independent runtime can checkpoint the stopped linked pair, retain a path-free tail checkpoint externally and restore only into empty roots with complete verification. | The store remains single writer and non-WORM. No deployed backup target, independently administered checkpoint, operator-fencing evidence, schedule, disposal process or disaster-recovery rehearsal exists. No attestation exists for an unmerged local candidate. |
| RK23 supply-chain compromise | The build materialises only Git-tracked allowlisted source, the exact approved cache record and the checksum-verified closed OKF projection; `.env` files and all other provider records are rejected and ignored. The canonical verifier binds the cache bytes, source, context, OCI blobs and closed runtime. Existing pinned tool, repeat-build, SBOM and vulnerability controls remain unchanged. | Registry publication and public-runtime provenance are absent. Vulnerability knowledge changes over time and must be rescanned; an unfixed finding is retained risk, not proof of safety. |
| RK25 telemetry leakage | No request logger is added. Acceptance makes one fixed catalogue-only call and rejects machine paths, bearer material and other sensitive text in bounded logs and artefacts. | TLS proxy, platform, backup and host telemetry require a separate privacy and retention review. |
