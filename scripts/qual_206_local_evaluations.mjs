...
const SUITE_DEFINITIONS = [
  ...
  },
  {
    ...
    testNames: [
      ...
      "inspects a durable public-read v2 receipt through the current v3 result",
    ],
    materials: [
  ...
  },
  {
    ...
    testNames: [
      ...
      "selects either the fixed loopback boundary or one canonical public HTTPS origin",
      "rejects widened or non-canonical public ingress origins",
    ...
    materials: [
      ...
      "apps/mcp-gateway/src/container-ingress.ts",
      "apps/mcp-gateway/src/container-main.ts",
      "apps/mcp-gateway/src/http-app.ts",
      "apps/mcp-gateway/src/http-server.ts",
      "apps/mcp-gateway/src/metadata.ts",
      "apps/mcp-gateway/src/openapi.ts",
      "apps/mcp-gateway/src/public-origin.ts",
      "apps/mcp-gateway/src/readiness-integrity.ts",
      "apps/mcp-gateway/openapi/catalogue-api.openapi.json",
    ],
  },
  {
    label: "governed-candidate-assembly",
    kind: "node-test",
    sourceTest: "apps/mcp-gateway/test/governed-assembly.test.ts",
    compiledTest: "apps/mcp-gateway/dist/test/governed-assembly.test.js",
    testNames: [
      "assembles one immutable exact-five candidate for direct, MCP HTTP and STDIO",
      "applies provider and tool suspension identically and fails readiness closed",
      "snapshots suspension inputs without invoking hostile traps or accessors",
      "rejects hostile or forged assembly authority and independent activation",
      "rejects every hostile governed wrapper option bag without invoking it",
      "keeps corrupt evidence readiness blocked without implying registration",
      "rechecks provider integrity before reporting candidate readiness",
      "locks every evidence dispatch surface against mid-call substitution",
      "uses the pristine provider execution seam on calls without a readiness probe",
      "keeps the Node candidate MCP face behind the same readiness guard",
      "rejects every public OpenAPI and admission-boundary divergence",
      "admits one exact public authority over the real Node transport",
      "keeps exact-five success, receipt and plain-text semantics across transports",
      "keeps v2 inspection identity separate from the recovered data receipt",
    ],
    materials: [
      "apps/mcp-gateway/src/catalogue-snapshot.ts",
      "apps/mcp-gateway/src/container-healthcheck.ts",
...
const CASE_ASSERTIONS = {
  ...
  E13: {
    ...
    assertions: [
      ...
      ["inspection-scope", "Inspection returns a distinct inline-only current-call receipt bound to the server-owned request and trace while preserving the earlier stored receipt and policy trace; it creates no ledger event and is not persisted or attested."],
    ],
    limitations: [
