...
export const MAX_CATALOGUE_QUERY_LENGTH = 256;
export const MAX_CATALOGUE_QUERY_TERMS = 10;
...
/**
 * Analyse a catalogue query once for every consumer.
 *
 * The Explorer retains its established behaviour by using the bounded `terms`
 * projection. Network transports must reject an analysis whose limit flags are
 * true before calling the shared search function.
 */
export function analyseCatalogueQuery(value: string): CatalogueQueryAnalysis {
  const normalised = normaliseSearchText(value);
  const inputTerms = normalised.split(" ").filter(Boolean);
  return {
    normalised,
    terms: inputTerms.slice(0, MAX_CATALOGUE_QUERY_TERMS),
    inputTermCount: inputTerms.length,
    // JSON Schema maxLength counts Unicode code points rather than UTF-16 units.
    exceedsCharacterLimit: Array.from(value).length > MAX_CATALOGUE_QUERY_LENGTH,
    exceedsTermLimit: inputTerms.length > MAX_CATALOGUE_QUERY_TERMS,
  };
}
...
export function searchRecords(
  ...
  const tokens = analyseCatalogueQuery(state.query).terms;
